...
- **Deploy (Crockis):** Assets-only Worker `crockis` (`crockis/wrangler.jsonc`). Build with `npm run build --prefix crockis/web`, then `npx wrangler deploy --config crockis/wrangler.jsonc`. Workflow `.github/workflows/deploy-crockis.yml` is `workflow_dispatch` only — never on `main` or pull requests. No Rust/WASM step.
