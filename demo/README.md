...
Leave `owner` commented in `app.yaml`. After the first deploy, set `AUTH_JWT_ACCESS_SECRET`, `AUTH_JWT_VERIFY_SECRET`, and `AUTH_HASH_PEPPER` as app secrets. Edge injects `DB_*` for the managed MySQL database. A `pre-deployment` job runs the package `migrate` command once per deploy; the server does not migrate on startup or per request.
