...
wrangler (run_worker_first: ["/api/*"])
    │
    ▼
demo/api/worker/build/index.js   ← JS shim from worker-build
    │
    ▼
...
```
demo/api/
  Cargo.toml           # rundtisch-demo (routes + handlers, publish = false)
  src/
    lib.rs
    routes.rs
    handlers.rs
...
### Worker as a nested cdylib

`#[event(fetch)]` must live in a `cdylib` compiled by `worker-build`. The nested `worker/` crate depends on `rundtisch-demo` with the `cloudflare` feature and only wires the fetch event to `handle_fetch`.
...
Output: `demo/api/worker/build/index.js` + `demo/api/worker/build/index_bg.wasm`
...
|---------|---------|--------|---------|
| `worker-build` | debug | `demo/api/worker/build/` | `wrangler.dev.jsonc`, local dev |
| `worker-build --release` | release | `demo/api/worker/build/` | `wrangler.jsonc`, CI, production |
...
`demo/api/worker/build/` is gitignored and regenerated on every deploy.
