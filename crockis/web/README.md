...
Production deploy is the assets-only Cloudflare Worker `crockis` (`../wrangler.jsonc`). GitHub Actions **Deploy Crockis** runs on `main`, pull requests (preview alias `pr-<N>`), and `workflow_dispatch`.
